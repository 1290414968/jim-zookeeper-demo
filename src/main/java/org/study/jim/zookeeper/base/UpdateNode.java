...
public class UpdateNode implements Watcher {
    ...
    public static void main(String[] args) {
        try {
            ...
            //创建
            String  syncPath =  zooKeeper.create(path,"0".getBytes(),ZooDefs.Ids.OPEN_ACL_UNSAFE,CreateMode.PERSISTENT);
            System.err.println(syncPath);
            //更新
            zooKeeper.getData(path,true,null);
            // -1 参数，zookeeper会根据该值进行当前节点最新版本基础上的更新
            Stat stat  =  zooKeeper.setData(path,"1".getBytes(),-1);
            System.err.println(stat.getMzxid()+"|"+stat.getVersion());
            // 返回当前节点的最新版本号，然后将版本作为参数进行更新
            Stat stat2 = zooKeeper.setData(path,"2".getBytes(),stat.getVersion());
            // stat版本已过期，更新报错
